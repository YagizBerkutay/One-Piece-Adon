...
function decodeCp1254(buffer) {
  ...
  // Windows-1254 (CP1254) byte decoding
  const cp1254Map = {
...
allAssFiles.forEach(file => {
  ...
  const decodedStr = decodeCp1254(buf);

  // Write back clean UTF-8 string to file
  fs.writeFileSync(file, decodedStr, 'utf-8');
  fixedCount++;
});

console.log(`✅ ${fixedCount} adet .ass altyazı dosyası tam 100% temiz UTF-8 formatına dönüştürüldü ve kaydedildi!`);
